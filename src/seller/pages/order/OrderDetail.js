...
export default function OrderDetail() {
  ...
  const getOrder = async () => {
    try {
      const res = await apiOrder.getByIdSeller(orderId, {
        headers: {
      ...
      data.items.forEach((item) => {
        tot2 +=
          ((item.salePrice > 0 ? item.salePrice : item.price) - item.discount) *
          item.quantity;
      ...
      const hasFinalStatus = data?.orderStatus.some((item) =>
        [
          ...
          "SELLER_CANCELLED",
          "DELIVERD",
          "SELLER_PREPAIRED", // final for seller
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 font-sans">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-gray-200 flex justify-between items-center">
          ...
          <div>
            {!isCancelled &&
              getLatestStatus(order?.orderStatus).status !== "DELIVERD" && (
                <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
            ...
            <div>
              <span className="text-yellow-600 font-semibold">
                ...
                <span className="text-red-700">{order?.paymentType}</span>
              </span>
            </div>
          </div>
        </div>
                ...
                {order?.items.map((product, index) => {
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <td className="px-4 py-2 whitespace-nowrap text-sm font-semibold text-gray-900 text-red-500">
