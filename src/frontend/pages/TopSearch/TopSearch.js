...
function TopSearch() {
  ...
  const getProducts = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiProduct.getTopSearchProducts(page, productsPerPage);
      if (res.data.length === 0) {
        ...
        setProducts((prevProducts) => [...prevProducts, ...sorted1]);
  ...
  const pageTitle = "Tìm kiếm hàng đầu";
              ...
              <div className="p-3">
                ...
                {product.variants[0]?.salePrice > 0 ? (
                  <p className="text-md font-bold text-red-600 flex justify-between items-baseline mt-2">
                    ...
                    <span className="ml-2">
                      {(
                        product.variants[0].salePrice - product.discount
                ...
                <p className="text-xs text-gray-600 mt-1">
                  Lượt mua: {product.sold.toLocaleString()}
                </p>
                <p className="text-xs text-gray-600 mt-1">
                  Lượt xem: {product.views.toLocaleString()}
                </p>
              </div>
            </Link>
          );
