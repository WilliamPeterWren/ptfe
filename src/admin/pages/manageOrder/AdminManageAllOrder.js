...
export default function AdminManageAllOrder() {
  ...
  const getAllOrder = async (page = 0) => {
    try {
      const res = await apiOrder.adminGetAllOrderPagination(page, size, {
        headers: {
  ...
  const getOrderByStatus = async (status, currentPage) => {
    ...
    try {
      const res = await apiOrder.adminGetAllOrderPaginationStatus(
        status,
        currentPage,
        size,
        {
  ...
  const handleFindOrderById = async () => {
    if (orderCode.length > 5) {
      try {
        const res = await apiOrder.adminGetOneOrderById(orderCode, {
  ...
  const handleToOrderDetail = (id) => {
    navigate(`/admin/manageorder/order-detail/${id}`);
  };
                        ...
                        <tr key={`${order.id}-${productIndex}`}>
                          ...
                          {productIndex === 0 && (
                            <>
                              ...
                              >
                                <Link
                                  to={`/admin/manageorder/order-detail/${order.id}`}
                                >
                                  <button
