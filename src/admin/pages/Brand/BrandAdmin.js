...
function BrandAdmin() {
    ...
    const [deleteBrandItem, setDeleteBrandItem] = useState(false);
    ...
    useEffect(() => {
        apiBrand.getBrandPagination(page, limit).then(res => {
    ...
    }, [page, deleteBrandItem])

    const handleDeleteBrand = (id) => {

      apiProduct.getProductByBrandId(id).then(res => {
        console.log("res: ", res.data)   
        try{
          const productData = res.data.length;
          if(productData){
            toast.error(`Warning! Can't Delete because there's at least One product has this Brand`, {
              position: "top-right",
              autoClose: 2000,
            ...
            apiBrand.deleteBrandById(id).then(res => {
              try{          
                setDeleteBrandItem(id);     
                toast.warning(`Warning! Delete ${res.data.attributes.brand_name} from Database`, {
                  position: "top-right",
                  autoClose: 2000,
                  hideProgressBar: false,
                  closeOnClick: true,
  ...
  return (
    <div>
    <br/>
      <h1 className='ml-4'>Brand Admin</h1>
      
      <Link to="/admin/dashboard/brand-admin/add-brand">
      ...
      <table className="brand-admin-list ml-4">
        <thead>
          <tr>        
            <th>ID</th>
            <th>Name</th>
        ...
        <tbody>
        {
          brands.map(item => {
            return (
              <tr key={item.id}>  
                <td>{item.id}</td>           
                <td>{capitalizeFirstLetter(item.brand_name)}</td>
                <td>{item.slug}</td>
         
                <td>{capitalizeFirstLetter(item.address)}</td>
             
                <td>
                  <Link to={`/admin/dashboard/brand-admin/edit-brand/${item.id}`}>
                  ...
                  <button className='btn btn-white'  onClick={() => handleDeleteBrand(item.id)}>
                  ...
                  page > 1 && (
                    ...
                    <button className='btn btn-warning mr-3'>
                      <Link 
                        ...
                        to={`/admin/dashboard/brand-admin/${page-1}`} 
                  ...
                  
                  {
                    Array.from({length: pages}, (v, k) => k + 1).map((item,index) => (
                      <li key={index} className={item === page ? 'active' : '' } style={{marginRight: '10px'}}>
                        <Link to={`/admin/dashboard/brand-admin/${item}`}>
