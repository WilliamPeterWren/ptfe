...
export default function OrderData({ shipperid }) {
  const shipperAccessToken = Cookies.get("shipperAccessToken");
  ...
  const getAllOrder = async (page = 0) => {
    try {
      const res = await apiOrder.getOrderByShipperId(page, {
        headers: {
          Authorization: `Bearer ${shipperAccessToken}`,
    ...
    if (orderCode.length > 5) {
      try {
        const res = await apiOrder.getByIdShipper(orderCode, {
          headers: { Authorization: `Bearer ${shipperAccessToken}` },
        });
  ...
  const handleToOrderDetail = (id) => {
    navigate(`/admin/manageshipper/${shipperid}/order-detail/${id}`);
  };

  if (shipperAccessToken === undefined || shipperAccessToken === null) {
    return null;
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 p-4 font-sans">
      <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-200 bg-gray-50 gap-4 flex justify-between items-center">
                        ...
                        <tr key={`${order.id}-${productIndex}`}>
                          ...
                          {productIndex === 0 && (
                            <>
                              ...
                              >
                                <Link
                                  to={`/admin/manageshipper/${shipperid}/order-detail/${order.id}`}
                                >
                                  <button
